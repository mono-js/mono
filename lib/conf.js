...
export default function (srcDir) {
	// Environement
	const env = process.env.NODE_ENV || 'development'

	// Conf path
	const confPath = process.env.MONO_CONF_PATH || join(srcDir, 'conf')
	...
	files.forEach((filename) => {
		...
		if (!fileExists) {
			...
			return this.log.error(`Could not load config file: ${filePath}`)
		}
